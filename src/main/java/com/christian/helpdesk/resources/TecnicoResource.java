...
import com.christian.helpdesk.services.TecnicoService;
...
public class TecnicoResource {
	...
	@GetMapping(value = "/{id}")
	public ResponseEntity<TecnicoDTO> encontrarTecnicoPeloId(@PathVariable Integer id){
		Tecnico obj = service.findTecnicoById(id);
		return ResponseEntity.ok().body(new TecnicoDTO(obj));
	}
	
	@GetMapping
	public ResponseEntity<List<TecnicoDTO>> encontrarTodos() {
		List<Tecnico> list = service.findTodosTecnicos();
	...
	public ResponseEntity<TecnicoDTO> create(@Valid @RequestBody TecnicoDTO objDTO) {
		Tecnico newtecobj = service.create(objDTO);
